...
void SpaceWars::initGameClient(const char *host, const char *port)
{
	...
	Vessel *player1 = new Vessel(game_, entityManager_, 0, Vector2D(w - (w / 2), SDLGame::instance()->getWindowHeight() / 2), game_->getTextureMngr()->getTexture(Resources::Player1), SDLK_d, SDLK_a, SDLK_w, false, false, bulletsPool1, plasma);
	entityManager_->addEntity(player1);
	Vessel *player2 = new Vessel(game_, entityManager_, 1, Vector2D(w + (w / 2), SDLGame::instance()->getWindowHeight() / 2), game_->getTextureMngr()->getTexture(Resources::Player2), SDLK_RIGHT, SDLK_LEFT, SDLK_UP, false, true, bulletsPool1, plasma);
...
void SpaceWars::initServer(const char *host, const char *port)
{
	...
	Vessel *player1 = new Vessel(game_, entityManager_, 0, Vector2D(w - (w / 2), SDLGame::instance()->getWindowHeight() / 2),
								 game_->getTextureMngr()->getTexture(Resources::Player1), SDLK_RIGHT, SDLK_LEFT, SDLK_UP, true, true, bulletsPool1, plasma);
	entityManager_->addEntity(player1);
	Vessel *player2 = new Vessel(game_, entityManager_, 1, Vector2D(w + (w / 2), SDLGame::instance()->getWindowHeight() / 2),
	 game_->getTextureMngr()->getTexture(Resources::Player2), SDLK_d, SDLK_a, SDLK_w, true, false, bulletsPool1,plasma);
	entityManager_->addEntity(player2);
	logic_ = new GameLogic(player1, player2, asteroidPool_, bulletsPool1, plasma);
